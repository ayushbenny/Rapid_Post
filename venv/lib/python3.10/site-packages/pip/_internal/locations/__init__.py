...
def _looks_like_red_hat_scheme() -> bool:
    ...
    return (
        cmd.exec_prefix == f"{os.path.normpath(sys.exec_prefix)}/local" and cmd.prefix == f"{os.path.normpath(sys.prefix)}/local"
    )
...
def _looks_like_msys2_mingw_scheme() -> bool:
    ...
    return all(
        "Lib" not in p and "lib" in p and not p.endswith("site-packages") for p in (paths[key] for key in ("platlib", "purelib"))
    )
